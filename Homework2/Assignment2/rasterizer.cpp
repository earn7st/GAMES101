...
static bool insideTriangle(float x, float y, const Vector3f* _v) //这块卡了半天，框架里坐标是 int，如果要 SSAA 的话显然不能 int(就算是正常的以像素中心取值也不能int) -w-
{   
    // TODO : Implement this function to check if the point (x, y) is inside the triangle represented by _v[0], _v[1], _v[2]
    Eigen::Vector3f temp, p;
    bool d[3];
    for(int i = 0; i < 3; i ++){
        temp << _v[(i + 1) % 3].x() - _v[i].x(), _v[(i + 1) % 3].y() - _v[i].y(), 0;
        p << x - _v[i].x(), y - _v[i].y(), 0;
        d[i] = temp.cross(p).z() > 0;
    }
